...
public class Result {
     ...
     * @param path the path of represented archive or file
     * @param archiv true if this represents an archive
     */
    public Result(Result parent, String path, boolean archiv) {
        this.parent = parent;
        this.path = path;
        this.archiv = archiv;
        if(parent != null) {
    ...
    public int getHitCount() {
        if(getLevel() != 0 && !archiv) {  // files only produce Results if they are hits
            return 1;
        }
        int counter = 0;
