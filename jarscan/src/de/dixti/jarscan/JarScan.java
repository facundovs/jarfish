...
public class JarScan {
    ...
    public JarScan(Options options) {
        ...
        if(options.isManifestScan()) {
            ...
            scanner = new FileScanner(searchString);
        }else if(options.isClassVersionScan()) {
            scanner = new ClassVersionScanner(options.getClassVersionString());
        }else {
        ...
        for (int i = 0; i < files.length; i++) {
            if (files[i].isDirectory()) {     // directory
                ...
                try {
                    ...
                    if(printer != null) {
                        printer.printResult(jarResult); // top level archives can be printed immediately
                    }
                    ...
                    if(hasSuffix(options.getJarSuffixList(), files[i].getName())) {  // damaged archive
                        ...
                        printer.printResult(jarResult); // top level files can be printed immediately
                    }
                    try {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //Options options = new Options(new String[] {"-d", "C:/projekt/jarscan/testjars", "test"});
